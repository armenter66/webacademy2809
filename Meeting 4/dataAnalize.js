...
function task2( data ) {
    ...
    const ageDiffs = families
        .map( function (family) {
            return Math.abs(family.mother.born - family.father.born);
        });
...
function task3( data ) {
    ...
    const children = getFamilies(data)
     .filter( function (family) {
            return family.children;
     });

    const familySum = getFamilies(data).length; 
    
    const childrenSum =  children.length;


    return childrenSum / familySum;
}

console.log( 'среднее количество детей в семье', task3( ANCESTRY_DATA ));